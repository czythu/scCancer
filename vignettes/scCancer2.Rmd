...
# path <- "D:/scCancer-data/SubtypeAnno/Demo-pipeline/GSE148673/"
matrix <- readRDS(paste0(path, "data/ori/CRA001160_expression_sparse.rds"))
# matrix <- t(readRDS(paste0(path, "data/ori/PAAD_GSE148673_expression.h5.RDS")))
gene.list <- read.table(paste0(system.file("txt", package = "scCancer"), "/single_cell_features.tsv"))[,1:2]
index <- which(rownames(matrix) %in% gene.list$V2)
matrix.sub <- matrix[index,]
geneset <- gene.list[match(rownames(matrix.sub), gene.list$V2),]
...
path <- "D:/scCancer-data/SubtypeAnno/Demo-pipeline/20180416-HCC-P2-Tumor/"
...
for (sample in samples){
    ...
    anno.results <- runScAnnotation(
        ...
        subtype.umap = T,
        bool.runMalignancy = T,
        malignancy.method = "both", # "xgboost", or "inferCNV", "both", recommend "both" for sample < 10000 cells
        bool.intraTumor = T,
        bool.runCellCycle = T,
        bool.runStemness = T,
        bool.runGeneSets = T,
